package org.telekit.base;

public interface MessageKeys {

    String CONFIRMATION = "Confirmation";
    String INFO = "Info";
    String PREFERENCES = "Preferences";
    String STATUS = "Status";
    String QUIT = "Quit";
    String WARNING = "Warning";

    String MSG_GENERIC_IO_ERROR = "msg.generic-io-error";
    String MSG_TASK_COMPLETED = "msg.task-completed";
    String MSG_TASK_CANCELED = "msg.task-canceled";
    String MSG_UNABLE_TO_SAVE_FILE = "msg.unable-to-save-file";
    String MSG_UNABLE_TO_IMPORT_DATA = "msg.unable-to-import-data";
    String MGG_UNABLE_TO_EXPORT_DATA = "msg.unable-to-export-data";
    String MGG_UNABLE_TO_EXTRACT_FILE = "msg.unable-to-extract-file";
    String MGG_UNABLE_TO_PARSE_CONFIG = "msg.unable-to-parse-config";
    String MGG_UNABLE_TO_SAVE_CONFIG = "msg.unable-to-save-config";

    String FILE_DIALOG_TEXT = "file-dialog.text";
    String FILE_DIALOG_XML = "file-dialog.xml";
    String FILE_DIALOG_ZIP = "file-dialog.zip";
}
