...
public final class SettingsTab extends Tab {
    ...
    public SettingsTab(ApiClientView view) {
        ...
        DefaultEventBus.getInstance().subscribe(CompletionRegistryUpdateEvent.class, e -> paramTable.refresh());
    }
    ...
    private TableView<Param> createParamTable() {
        ...
        MenuItem paramCompletionItem = createMenuItem(t(TOOLS_CHOOSE_VALUE), e -> showParamCompletions(), null);
        contextMenu.getItems().add(paramCompletionItem);
        paramCompletionItem.setVisible(false);
        selectionModel.selectedItemProperty().addListener(
                (obs, old, value) -> paramCompletionItem.setVisible(model.doesParamSupportCompletion(value))
        );
