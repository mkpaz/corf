...
import org.telekit.controls.util.IdentityObjectProperty;
...
public class ApiClientViewModel implements Initializable, ViewModel {
    ...
    public void selectFirstTemplate() {
        if (!templates.getItems().isEmpty()) {
            selectedTemplateProperty().set(templates.getItems().get(0));
        }
    }
    ...
    private final ObjectProperty<Template> selectedTemplate = new IdentityObjectProperty<>(this, "selectedTemplate");
